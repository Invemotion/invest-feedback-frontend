...
class _CalendarState extends State<Calendar> {
  ...
  Future<void> _loadMonth(DateTime focus) async {
    ...
    _byDate.clear();
    debugPrint("🟡 [_loadMonth] start month=$month");
  ...
  List<String> _getEventsForDay(DateTime day) {
    ...
    final hasTrade = true; // 기록이 있으면 매매 있음
    final hasJournal = list.any((e) {
      final hj = e['hasJournal'];
      final jid = e['journalId'];
      final hjTrue = (hj is bool) ? hj : (hj?.toString() == 'true');
      final hasId = jid != null && jid.toString().isNotEmpty;
      return hjTrue == true || hasId;
    });
                ...
                child: TableCalendar(
                  ...
                  onDaySelected: (sel, foc) async {
                    ...
                    final toShow = raw.map((e) => {
                      ...
                      // ✅ DayDetailPage가 저널 단건 조회 키로 쓸 값들 — 꼭 문자열로 전달
                      'hasJournal': ((e['hasJournal'] ?? false) as bool).toString(),
                      'journalId' : (e['journalId'] == null) ? '' : e['journalId'].toString(),
                    }).toList();
                ...
                child: Row(
                  children: [
                    DropdownButton<ReportType>(
                      ...
                      items: const [
                        DropdownMenuItem(value: ReportType.day, child: Text('일별 리포트', style: TextStyle(fontFamily: 'KBFGText', fontSize: 15))),
                        DropdownMenuItem(value: ReportType.month, child: Text('월별 리포트', style: TextStyle(fontFamily: 'KBFGText', fontSize: 15))),
                        DropdownMenuItem(value: ReportType.year, child: Text('연간 리포트', style: TextStyle(fontFamily: 'KBFGText', fontSize: 15))),
                      ],
                    ...
                    DropdownButton<int>(
                      ...
                      items: _years
                        ...
                        value: y,
                        child: Text('$y년', style: const TextStyle(fontFamily: 'KBFGText')),
                      ))
                          .toList(),
                    ...
                    if (_reportType != ReportType.year) ...[
                      ...
                      DropdownButton<int>(
                        ...
                        items: _months
                          ...
                          value: m,
                          child: Text('$m월', style: const TextStyle(fontFamily: 'KBFGText')),
                        ))
                            .toList(),
                    ...
                    if (_reportType == ReportType.day) ...[
                      const SizedBox(width: 6),
                      DropdownButton<int>(
                        value: _reportDate.day.clamp(1, _daysInMonth(_reportDate.year, _reportDate.month)),
                        underline: const SizedBox.shrink(),
                        items: List<int>.generate(_daysInMonth(_reportDate.year, _reportDate.month), (i) => i + 1)
                            .map((d) => DropdownMenuItem(
                          value: d,
                          child: Text('$d일', style: const TextStyle(fontFamily: 'KBFGText')),
                        ))
                            .toList(),
